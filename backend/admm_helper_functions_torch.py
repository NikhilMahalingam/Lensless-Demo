...
def to_complex(x):
    """Accept (...,2) real/imag or complex dtype; return complex dtype."""
    if torch.is_complex(x):
        return x
    return torch.complex(x[..., 0], x[..., 1])

def to_ri(xc):
    """complex dtype -> (...,2) real/imag stacked"""
    return torch.stack((xc.real, xc.imag), dim=-1)

def fft2_ri(x):
    """2D FFT. Input can be (...,2) or complex. Output is (...,2)."""
    xc = to_complex(x)
    Xc = torch.fft.fftn(xc, dim=(-2, -1))
    return to_ri(Xc)

def ifft2_ri(X):
    """2D IFFT. Input can be (...,2) or complex. Output is (...,2)."""
    Xc = to_complex(X)
    xc = torch.fft.ifftn(Xc, dim=(-2, -1))
    return to_ri(xc)
#
...
def complex_multiplication(t1, t2):
    """
    Supports:
      - real/imag stacked tensors with last dim = 2  (..,2)
      - complex dtype tensors (complex64/complex128)
    Returns a stacked real/imag tensor with last dim = 2 (to match the rest of this codebase).
    """
    # Convert t1 to complex
    if torch.is_complex(t1):
        t1c = t1
    else:
        t1c = torch.complex(t1[..., 0], t1[..., 1])

    # Convert t2 to complex
    if torch.is_complex(t2):
        t2c = t2
    else:
        t2c = torch.complex(t2[..., 0], t2[..., 1])
...
def Hfor(model, x):
    xc = torch.stack((x, torch.zeros_like(x, dtype=torch.float32)), -1)  # (...,2)
    X = fft2_ri(xc)                                                      # (...,2)
    HX = complex_multiplication(model.H, X)                               # (...,2)
    out = ifft2_ri(HX)                                                   # (...,2)
    out_r = out[..., 0]
    return out_r
def Hadj(model, x):
    ...
    xc = torch.stack((x, torch.zeros_like(x, dtype=torch.float32)), -1)  # (...,2)
    X = fft2_ri(xc)                                                     # (...,2)
    HX = complex_multiplication(model.Hconj, X)                          # (...,2)
    out = ifft2_ri(HX)                                                  # (...,2)
    out_r = out[..., 0]
    return out_r
