...
import sys
# PERCEPTUAL_SIM_PATH = os.path.join(os.path.dirname(__file__), "PerceptualSimilarity")
# sys.path.insert(0, PERCEPTUAL_SIM_PATH)
# from models import dist_model as dm
import lpips
import torch

# Create once and reuse (pick alex/vgg; alex is common default)
_lpips_model = None
...
def lpips_distance(img_a, img_b, device, net='alex'):
    """
    img_a/img_b: torch tensors shaped [N,3,H,W]
    IMPORTANT: LPIPS expects RGB normalized to [-1, 1]
    """
    loss_fn = get_lpips(device, net=net)

    # If your tensors are in [0,1], convert to [-1,1]
    if img_a.min() >= 0 and img_a.max() <= 1:
        img_a = img_a * 2 - 1
    if img_b.min() >= 0 and img_b.max() <= 1:
        img_b = img_b * 2 - 1

    with torch.no_grad():
        return loss_fn(img_a.to(device), img_b.to(device))
